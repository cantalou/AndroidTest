...
public class ProxyResources extends Resources {
	 ...
	 * Drawable资源缓存(图片,xml)
	 */
	protected LongSparseArray<WeakReference<ConstantState>> drawableCache;

	/**
	 * colorDrawable资源缓存(单色值)
	 */
	protected LongSparseArray<WeakReference<ConstantState>> colorDrawableCache;

	/**
	 * android >= 4.1 colorStateList资源缓存(多状态色值)
	 */
	protected LongSparseArray<WeakReference<ColorStateList>> colorStateListCache16;
	...
	public ProxyResources(String packageName, Resources skinRes, Resources defRes, String skinName) {
		...
		drawableCache = get(Resources.class, "sPreloadedDrawables");
		colorDrawableCache = get(Resources.class, "sPreloadedColorDrawables");
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
			colorStateListCache16 = get(Resources.class, "mPreloadedColorStateLists");
		} else {
			colorStateListCache = get(Resources.class, "mPreloadedColorStateLists");
		}
	}
	 ...
	 * 替换预加载缓存
	 * 
	 * @param preloadedDrawables
	 *            图片缓存
	 * @param preloadedColorDrawables
	 *            颜色缓存
	 * @param preloadedColorStateLists
	 *            小于Build.VERSION_CODES.JELLY_BEAN
	 * @param preloadedColorStateLists16
	 *            Build.VERSION_CODES.JELLY_BEAN 及其以上
	 */
	public void proxyPreloadResourcesCache(LongSparseArray<WeakReference<ConstantState>> preloadedDrawables,
			LongSparseArray<WeakReference<ConstantState>> preloadedColorDrawables,
			SparseArray<WeakReference<ColorStateList>> preloadedColorStateLists,
			LongSparseArray<WeakReference<ColorStateList>> preloadedColorStateLists16) {
		this.drawableCache = preloadedDrawables;
		this.colorDrawableCache = preloadedColorDrawables;
		this.colorStateListCache = preloadedColorStateLists;
		this.colorStateListCache16 = preloadedColorStateLists16;
	}
